...
const maybeSendResetLinkByTelegram = async (params) => {
    if (!params.telegramId || !Number.isFinite(params.telegramId))
        return false;
    try {
        const telegramPayload = (0, passwordReset_1.buildPasswordResetTelegramPayload)(params.resetUrl, params.expiresAt);
...
router.post('/change-password', auth_1.authMiddleware, async (req, res) => {
    try {
        ...
        const user = await dependencies_1.userRepository.findById(userId);
        ...
        await dependencies_1.userRepository.update(user.id, { password: hashed });
...
router.post('/password-reset/request-admin', auth_1.authMiddleware, async (req, res) => {
    try {
        ...
        const targetUser = requestedUserId ? await dependencies_1.userRepository.findById(requestedUserId) : await dependencies_1.userRepository.findByPhone(phone);
        if (!targetUser) {
            return res.status(404).json({ error: 'Usuario no encontrado' });
        }
...
router.get('/password-reset/validate', async (req, res) => {
    try {
        ...
        const valid = await (0, passwordReset_1.isPasswordResetTokenValid)(token);
...
router.post('/password-reset/confirm', async (req, res) => {
    try {
        ...
        const consumed = await (0, passwordReset_1.consumePasswordResetToken)(token);
        if (!consumed) {
            return res.status(400).json({ error: 'El enlace no es valido o ya expiro' });
        }
        const user = await dependencies_1.userRepository.findById(consumed.userId);
