...
import { computeAvailableStock } from '@inventory/domain/services/computeAvailableStock'
...
import {
  ...
  invoiceIssuerSettingsRepository,
  inventoryGateway,
...
const getBogotaCurrentWeekRange = (): DateRange => {
...
const getBogotaCurrentMonthRange = (): DateRange => {
  const today = getBogotaTodayUtc()
...
const isSalesMetricsQuery = (value: string) => {
  const normalized = normalizeText(value)
...
const toBogotaUtcRange = (period: DateRange) => ({
  from: new Date(`${period.start}T00:00:00.000-05:00`),
  to: new Date(`${period.end}T23:59:59.999-05:00`),
})

const toJournalSearchRange = (period: DateRange) => ({
  from: new Date(`${period.start}T00:00:00.000Z`),
  to: new Date(`${period.end}T23:59:59.999-05:00`),
})

const dayInPeriod = (day: string, period: DateRange) => day >= period.start && day <= period.end
...
const isJournalDateWithinPeriod = (value: Date, period: DateRange) => {
...
const getSaleAmountFromJournalEntry = (entry: SalesJournalEntryMetric) => {
  ...
  if (revenueDebit > 0) return revenueDebit
  ...
  const revenueCredit = entry.movements
    ...
    .reduce((sum, movement) => sum + (Number.isFinite(movement.amount) ? movement.amount : 0), 0)
  if (revenueCredit > 0) return revenueCredit

  const eventType = normalizeText(String(entry.eventType ?? ''))
  const description = normalizeText(String(entry.description ?? ''))
  const looksLikeSale = eventType === EventType.SALE || /\bventa\b/.test(description)
  if (!looksLikeSale) return 0
  ...
  if (debitAmounts.length === 0) return 0
  return Math.max(...debitAmounts)
}

const getSalesMetricsSummary = async (companyId: string, period: DateRange) => {
  ...
  while (true) {
    const movementPage = await inventoryGateway.listMovements({
      companyId,
      type: 'OUT',
    ...
    for (const movement of movementPage.items) {
      ...
      if (movement.reference.type !== 'SALE') continue
    ...
    if (movementPage.items.length === 0 || page * pageSize >= totalMovements) break
    page += 1
  }
  ...
  do {
    const pageResult = await journalEntryRepository.findPaginated({
      companyId,
      page: journalPage,
      limit: journalLimit,
  ...
  while (true) {
    ...
    for (const event of invoicePageResult.items) {
      const invoiceData = event.interpretedData as Partial<InvoiceSignatureData>
...
const buildGuidedVariantQtyAction = (qty: number) => `${GUIDED_VARIANT_ACTIONS.qtyPrefix}${qty}`

const parseGuidedVariantQtyAction = (action: string): number | null => {
  if (!action.startsWith(GUIDED_VARIANT_ACTIONS.qtyPrefix)) return null
...
const mapGuidedSaleCallbackActionToText = (action: string): string => {
  if (action === GUIDED_ACTIONS.yes) return 'si'
  if (action === GUIDED_ACTIONS.no) return 'no'
  if (action === GUIDED_ACTIONS.confirm) return 'confirmar'
  if (action === GUIDED_ACTIONS.cancel) return 'cancelar'
  if (action === GUIDED_ACTIONS.unit) return 'unitario'
  if (action === GUIDED_ACTIONS.total) return 'total'
  if (action === GUIDED_VARIANT_ACTIONS.manual) return VARIANT_QTY_MANUAL_INPUT_TOKEN
  if (action === GUIDED_VARIANT_ACTIONS.fillZeros) return VARIANT_QTY_FILL_ZERO_TOKEN
  const quickQty = parseGuidedVariantQtyAction(action)
  if (quickQty !== null) return String(quickQty)
  return ''
}
...
const buildGuidedVariantQtyKeyboard = (remaining: number | null, hasMoreVariants: boolean) => {
...
type GuidedSaleMetadata = {
  ...
  variantWizardValues?: number[] | null
  pendingPriceAmount?: number | null
  pendingPriceScope?: 'unit' | 'total' | null
  pendingCreditTermAmount?: number | null
  pendingCreditTermUnit?: 'days' | 'weeks' | 'months' | null
  fastMode?: boolean
...
type GuidedInvoiceIssuerStep = 'companyName' | 'taxId' | 'contactPhone' | 'address' | 'confirm'

type GuidedInvoiceIssuerMetadata = {
  step: GuidedInvoiceIssuerStep
}
...
type InventoryStockQuery =
  | { type: 'summary'; search?: string | null }
  | { type: 'low_stock'; threshold: number; search?: string | null }
...
const parseInventoryStockQuery = (value: string): InventoryStockQuery | null => {
  const text = normalizeText(value)
  ...
  const asksHowManyOwned =
    /\b(cuanto|cuanta|cuantos|cuantas)\b.*\b(tengo|quedan|queda)\b/.test(text) ||
    /\b(tengo|quedan|queda)\b.*\b(cuanto|cuanta|cuantos|cuantas)\b/.test(text)

  if (!mentionsInventory && !asksLowStock && !asksHowManyOwned) return null
  if (!asksQuantity && !asksLowStock) return null
...
const parseNonNegativeQuantity = (value: string): number | null => {
  const cleaned = value.trim()
...
const parseGuidedVariantPriceInput = (value: string): number | null => {
  ...
  if (largest && largest > 0) return largest
...
const parseCreditDueTerm = (value: string): { amount: number; unit: 'days' | 'weeks' | 'months' } | null => {
  const text = normalizeText(value)
  if (!text) return null
...
const addCreditTermToDate = (baseDate: string, amount: number, unit: 'days' | 'weeks' | 'months'): string | null => {
...
const parseCreditDueInput = (
  value: string,
  saleDate: string | null | undefined,
): { dueDate?: string; termAmount?: number; termUnit?: 'days' | 'weeks' | 'months' } | null => {
...
const completeSignedInvoiceForPending = async (pending: PendingEvent, meta: InvoiceSignatureMeta): Promise<void> => {
  ...
  const companySettings = await invoiceIssuerSettingsRepository.getByCompanyId(companyId)
...
const getGuidedSaleState = (pending: PendingEvent): { data: GuidedSaleData; meta: GuidedSaleMetadata } => {
  ...
  const meta: GuidedSaleMetadata = {
    ...
    variantWizardValues: Array.isArray(rawMeta.variantWizardValues)
      ? (rawMeta.variantWizardValues as number[]).map((item) => Number(item)).filter((item) => Number.isFinite(item) && item >= 0)
      : null,
    ...
    pendingCreditTermUnit:
      rawMeta.pendingCreditTermUnit === 'days' || rawMeta.pendingCreditTermUnit === 'weeks' || rawMeta.pendingCreditTermUnit === 'months'
...
const askGuidedSaleVariantQuantityStep = async (
  chatId: number,
  productName: string,
...
const isSimpleGuidedVariant = (variant: GuidedSaleVariant) =>
  variant.variantId === SIMPLE_GUIDED_VARIANT_ID ||
  (normalizeText(variant.attribute) === SIMPLE_GUIDED_VARIANT_ATTRIBUTE && normalizeText(variant.value) === SIMPLE_GUIDED_VARIANT_VALUE)

const askGuidedSaleQuantity = async (chatId: number, productName: string, suggestedQty?: number | null) => {
  const suggestion =
...
const moveGuidedSaleToQuantityStep = async (
  pendingId: string,
  chatId: number,
  data: GuidedSaleData,
...
const askGuidedSaleVariantPriceStep = async (
  chatId: number,
  productName: string,
  variants: Array<{ id: string; attribute: string; value: string }>,
  prices: number[],
...
const toSafeQty = (value: unknown): number | null => {
  const parsed = typeof value === 'number' ? value : typeof value === 'string' ? Number(value) : Number.NaN
  if (!Number.isFinite(parsed)) return null
  return Math.max(0, Math.round(parsed))
}

const formatInventoryConfirmErrorMessage = (data: GuidedSaleData, error: InventoryConfirmError): string => {
  ...
  if (requestedQty !== null) lines.push(`Solicitado: ${requestedQty}`)
  if (availableQty !== null) lines.push(`Disponible: ${availableQty}`)
  if (missingQty !== null) lines.push(`Faltan: ${missingQty}`)
...
const resetGuidedVariantWizard = (meta: GuidedSaleMetadata, totalVariants: number) => {
  meta.variantWizardIndex = 0
  meta.variantWizardValues = Array.from({ length: totalVariants }, () => 0)
}

const clearGuidedVariantWizard = (meta: GuidedSaleMetadata) => {
  meta.variantWizardIndex = null
  meta.variantWizardValues = null
}

const resetGuidedPriceVariantWizard = (
  meta: GuidedSaleMetadata,
  variants: Array<{ id: string; attribute: string; value: string }>,
  item: GuidedSaleItem | null,
) => {
...
const applyGuidedVariantSelection = async (
  pendingId: string,
  chatId: number,
  data: GuidedSaleData,
  meta: GuidedSaleMetadata,
...
const continueAfterGuidedVariantPrices = async (pendingId: string, chatId: number, data: GuidedSaleData, meta: GuidedSaleMetadata) => {
  if (meta.fastMode) {
...
const getGuidedInvoiceIssuerState = (pending: PendingEvent): { data: GuidedInvoiceIssuerData; meta: GuidedInvoiceIssuerMetadata } => {
  const rawData = pending.interpretedData ?? {}
  const rawMeta = pending.metadata ?? {}

  const companyName = typeof rawData.companyName === 'string' ? rawData.companyName : rawData.companyName === null ? null : undefined
  const taxId =
    typeof rawData.taxId === 'string'
      ? rawData.taxId
      : rawData.taxId === null
        ? null
  ...
  return {
    ...
    meta: { step: (rawMeta.step as GuidedInvoiceIssuerStep) ?? 'companyName' },
  }
}

const updateGuidedInvoiceIssuerState = async (pendingId: string, data: GuidedInvoiceIssuerData, meta: GuidedInvoiceIssuerMetadata) => {
  await pendingEventRepository.updateData(pendingId, data as Record<string, unknown>, meta as Record<string, unknown>)
}

const isValidInvoiceIssuerCompanyName = (value: string | null | undefined) => {
  if (!value) return false
  return value.trim().length >= 2
}

const isValidInvoiceIssuerTaxId = (value: string | null | undefined) => {
  if (!value) return false
  const normalized = normalizeCustomerDocumentInput(value)
  const alphanumericLength = normalized.replace(/[^a-zA-Z0-9]/g, '').length
  return alphanumericLength >= 5
...
const askGuidedInvoiceIssuerCompanyName = async (chatId: number) => {
  await TelegramClient.sendMessage({
    chatId,
...
const askGuidedInvoiceIssuerTaxId = async (chatId: number) => {
  await TelegramClient.sendMessage({
    chatId,
...
const askGuidedInvoiceIssuerContactPhone = async (chatId: number) => {
  await TelegramClient.sendMessage({
    chatId,
...
const askGuidedInvoiceIssuerAddress = async (chatId: number) => {
  await TelegramClient.sendMessage({
    chatId,
...
const buildGuidedInvoiceIssuerSummary = (data: GuidedInvoiceIssuerData) =>
  [
...
const saveInvoiceIssuerSettingsFromInput = async (chatId: number, companyId: string, data: GuidedInvoiceIssuerData): Promise<boolean> => {
...
const startGuidedInvoiceIssuer = async (chatId: number, companyId: string) => {
  const existing = await pendingEventRepository.findLatestPendingByTelegramUserId(chatId, 'invoice_issuer_guided')
  if (existing && existing.status === 'PENDING_CONFIRMATION') {
    await pendingEventRepository.updateStatus(existing.id, 'CANCELLED')
  }

  const current = await invoiceIssuerSettingsRepository.getByCompanyId(companyId)
  const data: GuidedInvoiceIssuerData = {
    companyName: current?.companyName ?? null,
    taxId: current?.taxId ?? null,
    contactPhone: current?.contactPhone ?? null,
    address: current?.address ?? null,
  }
  const meta: GuidedInvoiceIssuerMetadata = { step: 'companyName' }
...
const handleGuidedInvoiceIssuerMessage = async (pending: PendingEvent, chatId: number, rawText: string) => {
  const text = rawText.trim()
  const normalized = normalizeText(text)
...
const handleGuidedInvoiceIssuerCallback = async (pending: PendingEvent, chatId: number, action: string) => {
  if (action === GUIDED_ACTIONS.cancel) {
    await pendingEventRepository.updateStatus(pending.id, 'CANCELLED')
    await TelegramClient.sendMessage({ chatId, text: 'Operacion cancelada.' })
    return true
...
const startGuidedSaleWithPrefill = async (chatId: number, saleInput: SaleEventInput, productNameGuess?: string | null) => {
  ...
  const inventoryMode = await inventoryGateway.getInventoryMode(saleInput.companyId)

  let step: GuidedSaleStep = 'product'
  const meta: GuidedSaleMetadata = { step, inventoryMode }
...
const buildInventorySearchTokens = (value: string) =>
  normalizeText(value)
    .split(/[\s,.;:!?]+/)
    .map((token) => {
      let normalized = token.replace(/[^a-z0-9]/g, '')
      if (normalized.endsWith('es') && normalized.length > 3) normalized = normalized.slice(0, -2)
      if (normalized.endsWith('s') && normalized.length > 3) normalized = normalized.slice(0, -1)
      return normalized
    })
    .filter((token) => token && !INVENTORY_QUERY_STOPWORDS.has(token))

const rowMatchesInventorySearch = (row: InventoryStockRow, search?: string | null) => {
...
const getInventoryStockRows = async (companyId: string): Promise<InventoryStockRow[]> => {
  const mode = await inventoryGateway.getInventoryMode(companyId)
  const products = await listAllActiveProducts(companyId)
  const rows: InventoryStockRow[] = []

  for (const product of products) {
    if (mode === 'SIMPLE') {
      const movements = await inventoryGateway.listMovements({
        companyId,
        productId: product.id,
        page: 1,
        pageSize: 10000,
      })
      const reservedQty = await inventoryGateway.listActiveReservedQtyByProduct(companyId, product.id.toString())
      const stock = computeAvailableStock(movements.items, reservedQty)
      rows.push({
    ...
    const variants = await inventoryGateway.listVariantsByProductId(companyId, product.id.toString())
    for (const variant of variants.filter((entry) => entry.active && entry.systemType !== 'SIMPLE_DEFAULT')) {
      ...
      const reservedQty = await inventoryGateway.listActiveReservedQtyByVariant(companyId, variant.id.toString())
      const stock = computeAvailableStock(movements.items, reservedQty)
      rows.push({
        productId: product.id.toString(),
        productName: product.name,
...
const sendInventoryStockMessage = async (chatId: number, companyId: string, query: InventoryStockQuery) => {
  ...
  const grouped = new Map<string, { productName: string; availableQty: number; reservedQty: number; rows: InventoryStockRow[] }>()
  ...
  const detailLines = visible.map((item) => {
    ...
    if (item.rows.length === 1 || !query.search) return `- ${item.productName}: ${item.availableQty} disponibles${reserved}`
...
const handleGuidedSaleMessage = async (pending: PendingEvent, chatId: number, rawText: string) => {
  ...
  if (!meta.inventoryMode) {
    meta.inventoryMode = await inventoryGateway.getInventoryMode(pending.companyId)
  }
    ...
    if (meta.step === 'payment') {
      ...
      if (date) {
        ...
        if (meta.pendingCreditTermAmount && meta.pendingCreditTermUnit) {
          ...
          if (dueDate) data.creditDueDate = dueDate
          meta.pendingCreditTermAmount = null
          meta.pendingCreditTermUnit = null
        }
      }
  ...
  if (meta.step === 'variants' && meta.currentProductId && meta.currentProductName && meta.candidateVariants?.length) {
    ...
    if (wizardEnabled) {
      ...
      if (currentIndex >= totalVariants) currentIndex = totalVariants - 1
      ...
      const finalizeWizard = async () => {
        ...
        if (capturedVariants.length === 0) {
          ...
          await updateGuidedSaleState(pending.id, data, meta)
          await askGuidedSaleVariantQuantityStep(
            chatId,
            currentProductName,
            candidateVariants,
            0,
            meta.variantWizardValues ?? [],
            meta.pendingTotalQty ?? null,
          )
          return true
        }
  ...
  if (meta.step === 'price_mode') {
    ...
    if (meta.currentProductName && meta.candidateVariants?.length) {
      ...
      await askGuidedSaleVariantPriceStep(
        chatId,
        meta.currentProductName,
        meta.candidateVariants,
        meta.variantWizardValues ?? [],
        meta.variantWizardIndex ?? 0,
      )
    } else {
  ...
  if (meta.step === 'price_variants') {
    ...
    if (wizardEnabled) {
      ...
      let currentIndex = Math.trunc(meta.variantWizardIndex ?? 0)
      if (currentIndex < 0) currentIndex = 0
      if (currentIndex >= candidateVariants.length) currentIndex = candidateVariants.length - 1
      ...
      targetItem.variants = targetItem.variants.map((variant) =>
        variant.variantId === currentVariant.id ? { ...variant, unitPrice: nextPrice } : variant,
      )
  ...
  if (meta.step === 'confirm') {
    ...
    if (!confirmResult.ok) {
      const error = confirmResult.error as InventoryConfirmError
      await TelegramClient.sendMessage({
        chatId,
    ...
    const companySettings = await invoiceIssuerSettingsRepository.getByCompanyId(pending.companyId)
...
router.get('/sign-session/:pendingId', async (req: Request, res: Response) => {
  ...
  const groupedProducts = new Map<
    string,
    {
      name: string
      subtotal: number
      lines: Array<{ label: string; qty: number; unitPrice: number; subtotal: number }>
    }
  >()
  ...
  try {
    ...
    if (update?.callback_query) {
      ...
      if (guidedParsed && ensureChatId(chatId, res)) {
        ...
        const pendingInvoiceIssuer = await pendingEventRepository.findLatestPendingByTelegramUserId(chatId, 'invoice_issuer_guided')
    ...
    if (textForCommand && chatId) {
      ...
      const guidedInvoiceIssuer = await pendingEventRepository.findLatestPendingByTelegramUserId(chatId, 'invoice_issuer_guided')
      ...
      if (textForCommand && isSalesMetricsQuery(textForCommand)) {
        ...
        try {
          ...
          await TelegramClient.sendMessage({ chatId, text: `No pude calcular las ventas: ${message}` })
          return res.status(200).json({ ok: true })
        }
      }
      ...
      if (textForCommand) {
        ...
        if (inventoryQuery) {
          ...
          try {
            ...
            await TelegramClient.sendMessage({ chatId, text: `No pude consultar inventario: ${message}` })
          }
          return res.status(200).json({ ok: true })
        }
      }
