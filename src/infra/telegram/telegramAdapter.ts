...
const parseNumericDate = (raw: string): Date | null => {
...
const parsePaymentDateFromText = (rawText: string): string => {
  const text = rawText.toLowerCase()
  const today = getBogotaTodayUtc()

  if (/\bhoy\b/.test(text)) return toDateString(today)
  ...
  if (explicitDateMatch) {
    ...
    if (date) return toDateString(date)
  }
...
const extractExplicitSaleQuantity = (rawText: string): number | null => {
  const patterns = [
...
const extractPackSizeHint = (rawText: string): number | null => {
  const packMatch = rawText.match(/(?:^|[\s(])x\s*(\d{1,3})(?=\b|[^\d])/i)
  const packSize = toPositiveInt(packMatch?.[1])
  if (packSize && packSize > 1) return packSize

  if (/\bdocenas?\b/i.test(rawText)) return 12
  return null
}

const normalizeParsedSaleQuantity = (rawText: string, parsedQty: unknown): number | undefined => {
  const explicitQty = extractExplicitSaleQuantity(rawText)
  const normalizedParsedQty = toPositiveInt(parsedQty)

  if (!normalizedParsedQty) return explicitQty ?? undefined

  const packSize = extractPackSizeHint(rawText)
  if (!explicitQty || !packSize) return normalizedParsedQty
