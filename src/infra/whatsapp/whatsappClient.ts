...
const getApiUrl = (path: string) => `${WHATSAPP_API_BASE}/${WHATSAPP_API_VERSION}/${path}`

const sanitizePhoneNumber = (value: string): string => value.replace(/\D/g, '')

const hasCredentials = (): boolean => Boolean(WHATSAPP_ACCESS_TOKEN && WHATSAPP_PHONE_NUMBER_ID)

const uploadDocument = async (file: Buffer, filename: string): Promise<string> => {
...
const sendDocumentByMediaId = async (to: string, mediaId: string, filename: string, caption?: string): Promise<void> => {
  if (!WHATSAPP_PHONE_NUMBER_ID || !WHATSAPP_ACCESS_TOKEN) {
    throw new Error('WhatsApp credentials are not configured')
  }
...
export const WhatsAppClient = {
  ...
  async sendInvoiceDocument(payload: {
    ...
    filename: string
    caption?: string
  }): Promise<{ ok: boolean; reason?: 'not_configured' | 'invalid_phone' | 'send_error'; error?: string }> {
    if (!hasCredentials()) {
